...
function CandidateCards({ onSelect }) {
  return <section className="section" id="candidates"><div className="section-heading"><div><span className="kicker">{election.title}</span><h2>候補者から公約の経過を見る</h2></div><p>{election.disclaimer}。</p></div><div className="candidate-grid">{candidates.map(candidate=><article className="candidate-card" key={candidate.id}><CandidatePortrait candidate={candidate}/><div><span className="candidate-role">{candidate.role}・架空候補者</span><h3>{candidate.name}</h3><p>{candidate.firstTime ? '今回が初回立候補のため、過去公約との比較対象はありません' : '前回公約から現在までを、4つの政策テーマで確認'}</p></div><button onClick={()=>onSelect(candidate.id)}>公約の経過を見る <Arrow /></button></article>)}</div></section>;
}
...
function HeroMapPreview({ onOpen }) {
  const points = [
    {label:'子育て',x:322,y:56,lx:286,ly:43,color:'#c56f2a'},
    {label:'交通',x:252,y:112,lx:264,ly:129,color:'#8d7766'},
    {label:'医療',x:344,y:166,lx:306,ly:185,color:'#6f7477'},
    {label:'デジタル行政',x:188,y:84,lx:103,ly:72,color:'#b48a66'},
  ];
  return <aside className="hero-map-preview" aria-label="RMPM簡略プレビュー"><span className="hero-map-kicker">RMPM PREVIEW</span><h2>公約の履歴を、位置で読む。</h2><svg viewBox="0 0 420 320" role="img" aria-labelledby="hero-map-title hero-map-desc" preserveAspectRatio="xMidYMid meet"><title id="hero-map-title">4つの政策を配置したRMPMの簡略図</title><desc id="hero-map-desc">横軸は公開資料で確認できる記録の範囲、縦軸は公約全体における政策の中心性です。</desc><g className="preview-grid"><line x1="76" y1="34" x2="390" y2="34"/><line x1="76" y1="136" x2="390" y2="136"/><line x1="76" y1="238" x2="390" y2="238"/><line x1="76" y1="34" x2="76" y2="238"/><line x1="233" y1="34" x2="233" y2="238"/><line x1="390" y1="34" x2="390" y2="238"/></g><g className="preview-axes"><line x1="76" y1="238" x2="390" y2="238"/><line x1="76" y1="34" x2="76" y2="238"/><text x="76" y="258" textAnchor="middle">限定的</text><text x="233" y="258" textAnchor="middle">複数</text><text x="390" y="258" textAnchor="middle">継続的</text><text x="64" y="239" textAnchor="end">関連</text><text x="64" y="140" textAnchor="end">重点</text><text x="64" y="38" textAnchor="end">中心</text><text className="preview-x-title" x="233" y="293" textAnchor="middle">公開資料で確認できる記録の範囲 →</text><text className="preview-y-title" x="17" y="136" textAnchor="middle" transform="rotate(-90 17 136)">公約全体における政策の中心性 →</text></g>{points.map(point=><g className="preview-point" key={point.label}><line x1={point.x} y1={point.y} x2={point.lx} y2={point.ly+3}/><circle cx={point.x} cy={point.y} r="6" fill={point.color}/><text x={point.lx} y={point.ly}>{point.label}</text></g>)}</svg><p>点は、経過と理由を確かめるための入口です。</p><button onClick={onOpen}>マップの見方を見る <Arrow/></button></aside>;
}

function Home({ onSelect, setView }) {
  return <><section className="hero" id="top"><div className="hero-copy"><div className="eyebrow">選挙をまたいで、公約の経過をつなぐ</div><h1>その公約、<br/>前の選挙から<br/><em>何があった？</em></h1><p className="lead">過去公約から活動、現在公約、本人説明、メディア検証までを、次回選挙まで切らさない。</p><p className="hero-summary">選挙のたびに情報をリセットせず、政策がいつ、どう変わり、なぜ変わったのかを一つの流れで確認できる政治情報基盤です。</p><div className="hero-actions"><a className="button primary" href="#candidates">公約の経過を見る <span>↓</span></a><button className="text-link" onClick={()=>setView('method')}>仕組みと方法論を見る</button></div></div><HeroMapPreview onOpen={()=>onSelect('tanaka')}/></section><section className="hero-principle"><span>RMPMの基本原則</span><strong>RMPMは候補者を採点する仕組みではありません。</strong><p>公開記録、本人説明、取材をつなぎ、最終判断を有権者に委ねます。確認できる記録は政治活動のすべてではありません。</p></section><div className="fiction-banner">{election.disclaimer}。特定の実在政治家・自治体を評価するものではありません。</div><AccountabilityLoop/><CandidateCards onSelect={onSelect}/><TransparencyOverview compact/><section className="principles"><span className="kicker">次回選挙までつなぐ</span><h2>選挙が終わっても、公約の検証は続く。</h2><p>過去公約、確認可能な活動、現在の方針、本人説明、メディア取材を一つの履歴として残し、次の選挙で再び確かめられるようにします。</p><button onClick={()=>setView('method')}>方法論と限界を読む <Arrow/></button></section><PrototypeTest/></>;
}
...
function PolicyFlow({ policy, candidate, onSource }) {
  ...
  return <article className="policy-flow"><div className="flow-head"><div><span className="theme-chip">{policy.theme}</span><h3>{policy.current}</h3><span className="responsibility">主な責任主体：{responsibilityFor(policy.theme)}</span></div>{policy.status && <span className="status-chip">{policy.status}</span>}</div>
    ...
    <div className="evidence-sequence">
      ...
      <section className="sequence-card wide archive-card"><div className="role-section-heading"><RoleIcon role="reporter"/><div><span className="role-label">記者・メディア</span><span className="step-tag">07 過去発言・報道アーカイブとの照合</span></div></div><span className="fiction-badge">架空のアーカイブ</span><ArchivePanel policy={policy} onSource={onSource}/></section>
...
function CandidateDetail({ candidateId, onBack, onMethodology }) {
